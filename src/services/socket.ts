...
class SocketService {
  ...
  async connectAsRole(role: UserRole): Promise<Socket> {
    ...
    try {
      ...
      console.log("AsyncStorage에서 가져온 토큰:", accessToken ? `${accessToken.substring(0, 20)}...` : "토큰 없음")
      ...
      // 🔥 역할에 따른 엔드포인트 설정
      const endpoint = role === "host" ? "/host/location" : "/member/location"
      ...
      return new Promise((resolve, reject) => {
        ...
        this.socket!.on("connect", () => {
          ...
          console.log(`✅ ${role} 소켓 연결 성공 (${socketUrl}):`, this.socket?.id)
          resolve(this.socket!)
        })
  ...
  private setupEventListeners() {
    ...
    this.socket.on("connect_error", (error) => {
      console.error(`❌ ${this.currentRole} Socket connection error:`, error)
    })
  }
